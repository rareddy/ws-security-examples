...
class SubjectActions
{
   ...
   static class AddCredentialsAction implements PrivilegedAction<Void>
   {
      Subject subject;
      Serializable cred;
      AddCredentialsAction(Subject subject, Serializable cred)
      {
   ...
   static void addCredentials(Subject subject, Serializable cred)
   {
   ...
   private static Group addSubjectRoles(Subject theSubject, Set<Principal> roles)
   {
      ...
      while (iter.hasNext())
      {
         ...
         if (name.equals("Roles")) { //$NON-NLS-1$
			roleGrp = grp;
		}
      }
      ...
      if (roleGrp == null)
      {
         roleGrp = new SimpleGroup("Roles"); //$NON-NLS-1$
